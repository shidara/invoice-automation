...
features/liff/      LIFF（LINE内起動）の初期化Provider
components/          共通UI（必要になったら追加）
lib/                共通処理
  ├─ theme.ts            MUIテーマ（MD3ベース）
  ├─ format.ts           金額フォーマット
  ├─ date.ts             日付ユーティリティ
  ├─ liff.ts             LIFF初期化・LINE内判定
  └─ pdf/                PDFの生成と配信
      ├─ createInvoicePdfBlob.tsx  フォント登録＋PDF Blob生成
      ├─ deliverInvoicePdf.ts      配信方式の選択（通常DL / LINE内表示）
      ├─ downloadInvoicePdf.ts     通常ダウンロード
      └─ openInvoicePdf.ts         Viewer表示（LINE内向け）
stories/            Storybook
styles/             最小限のCSS
docs/               ドキュメント・画像
...
- `lib/pdf/deliverInvoicePdf.ts` … 配信方式の選択（通常DL / LINE内表示）。UIは生成→配信を呼ぶだけ
- `lib/pdf/downloadInvoicePdf.ts` … 通常ダウンロード（`invoice-{invoiceNumber}.pdf`）
- `lib/pdf/openInvoicePdf.ts` … Viewer表示（LINE内向け、失敗時はDLへフォールバック）
...
このアプリは LINE アプリ内（LIFF）でも起動できます。今回対応しているのは **LINE内で動かす基盤のみ**です。

セットアップ:

1. LINE Developers で Provider と LINE Login チャネルを作成
2. LIFF アプリを追加（Endpoint URL = 上記の公開URL、size = Full）
3. 発行された LIFF ID を環境変数 `NEXT_PUBLIC_LIFF_ID` に設定
   - ローカル: `.env.local`
   - GitHub Pages: リポジトリの Settings → Secrets and variables → Actions → **Variables** に `NEXT_PUBLIC_LIFF_ID` を登録（`deploy.yml` がビルド時に渡す）

挙動:

- `NEXT_PUBLIC_LIFF_ID` 未設定なら LIFF 初期化はスキップし、通常の Web アプリとして動作する
- LINE 内では PDF を Viewer 表示（ダウンロードが不安定な環境向け）、通常ブラウザではダウンロード
- 対象外（今後の課題）: トークへの共有、プロフィール取得による自動入力、正式な「LINE Mini App」プロダクト化（審査が必要）
